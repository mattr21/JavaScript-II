...
ticketPriceTotal = runners.reduce((total, current) => {
...
// Problem 1
//get an email list of every runner
emailsList = runners.map(runner => runner.email.toLowerCase());
console.log(emailsList)

// Problem 2
// find all runners that donate 200 or more
let topDonators = runners.filter(runner => runner.donation >= 200)
topDonatorsEmails = topDonators.map(runner => runner.email.toLowerCase());
console.log(topDonators);
console.log(topDonatorsEmails)

// Problem 3
//find donation average
let donationAvg = [];

donationAvg = runners.reduce((total, current) => {
    return total += current.donation;
}, 0)

console.log(ticketPriceTotal / (runners.length-1));